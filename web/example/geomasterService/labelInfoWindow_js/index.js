...
const poiSearch = async poi_id => {
  ...
  const response = await fetch(
    `https://gis.kt.com/search/v1.0/pois/${poi_id}?mode=NAVIGATION`,
  ...
  const homepage =
    poi.extension && poi.extension.homepageURL ? poi.extension.homepageURL : ""
  const roadAddress =
    poi.address.siDo +
    " " +
    poi.address.siGunGu +
  ...
  const photoURL =
    poi.extension && poi.extension.photoURL ? poi.extension.photoURL : ""
			...
			<div style="display:flex">
				<div>
					<div class="info">
						...
						<span class="tel"></span>
						<a href="${homepage}" target="_blank" class="web" style="display:${
    homepage === "" ? "none" : "inline"
  }">홈페이지</a>
					</div>
