...
async function getMiddle(selectedMaster: string) {
...
async function getSub(selectedMiddle: string) {
...
async function searchCategory(selectedSubCode: string) {
  ...
  // 검색 결과를 우측 결과창에 표출합니다
  ;(document.getElementById('features') as HTMLElement).innerHTML = JSON.stringify(place.pois, null, 2)
}

// select에 option 태그를 추가하는 함수입니다
function appendOption(text?: string, code?: string, parentChild?: Element | HTMLElement, disabled: boolean = false) {
