...
searchBtn.addEventListener('click', async () => {
  ...
  if (map.getLayer('EVStationLayer')) map.removeLayer('EVStationLayer')
  if (map.getSource('geojsonSource_point')) map.removeSource('geojsonSource_point')

  // 현재 지도의 bound(경계좌표)를 가져옵니다.
  let bound = map.getBound()

  // 현재 bound로 EVStation 검색합니다.
  const data = await searchService.evStation({ numberOfResults: 50, filters: { bound: { right: bound._ne.lng, top: bound._ne.lat, left: bound._sw.lng, bottom: bound._sw.lat } } })
  ...
  // 전기차충전소 좌표와 전기차충전소 이름을 pointGeo 형태의 배열로 저장합니다
  const geometry = []
  ...
  let source_point = new ktGms.source.GeoJSONSource('geojsonSource_point', {
    data: geometry,
    maxzoom: 18,
    cluster: true,
  ...
  map.onLayer("click", "EVStationLayer", (event) => {
    alert(event.features[0].properties.name);
  });

  // 클러스터 된 레이어의 개별 이름을 클릭했을 때 이름이 뜨도록 설정합니다
  map.onLayer("leaveClick", "EVStationLayer", (event) => {
