...
async function moveMap(selectedSido: string, selectedSigungu: string, selectedEupmyeondong: string) {
  // searchService.stepByStep를 사용하여 선택한 주소에 맞는 데이터를 가져옵니다
  const eupmyeondongResult = (await searchService.stepByStep({ siDo: selectedSido, siGunGu: selectedSigungu, eupMyeonDong: selectedEupmyeondong })) as StepByStepResponse
...
function appendOption(text?: string, parentChild?: Element | HTMLElement, disabled: boolean = false) {
